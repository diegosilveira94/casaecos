...
class FakePersonRepository implements PersonRepository {
  ...
  roleAvailable = true;
  phoneInUse = false;
  links: PersonLinks | null = { events: false, homes: false };
...
describe('PersonService', () => {
  ...
  it('recusa papel inexistente ao criar ou editar', async () => {
    ...
    await expect(service.create({ name: 'Maria', roleId: 99 })).rejects.toMatchObject({
      status: 400,
      message: 'Papel informado não existe',
    });
    await expect(service.update(1, { roleId: 99 })).rejects.toMatchObject({
      status: 400,
      message: 'Papel informado não existe',
    });
  });
  ...
  it('recusa telefone duplicado, inclusive em conflito concorrente no banco', async () => {
    ...
    await expect(
      service.create({ name: 'Maria', roleId: 2, phone: '47999999999' }),
  ...
  it('edita somente os campos informados e ignora o próprio telefone na unicidade', async () => {
    await service.update(1, { name: '  Maria Souza ', phone: ' 47988888888 ' });
  ...
  it('impede excluir pessoa vinculada a eventos ou casas', async () => {
    repository.links = { events: true, homes: true };
  ...
  it('distingue pessoa inexistente e exclui pessoa sem vínculos', async () => {
    ...
    repository.links = { events: false, homes: false };
